...
def get_column(matrix, i):
    """Returns the ith column of a matrix as a list."""
    f = itemgetter(i)
    return map(f, matrix)


def is_in_window(motif_interval, atac_median, window_size) -> bool:
    """Returns whether the given motif interval is within a window of size"""
    start = int(motif_interval.start)
    end = int(motif_interval.end)
    if (end >= (atac_median - window_size) and end <= (atac_median + window_size)) or (
        start >= (atac_median - window_size) and start <= (atac_median + window_size)
    ):
        return True
    else:
        return False
    ...
    for atac_peak in atac_iter:
        ...
        while motifs_within_region:
            ...
            if is_in_window(motif_region, atac_median, H):
                g_H = g_H + 1
                tf_median = (
                    motif_region.start + (motif_region.end - motif_region.start) / 2
                )
                tf_distances.append(atac_median - tf_median)
...
def get_md_score(
    tf_motif_filename, mp_threads, atac_peaks_filename, CHROMOSOMES, radius
):
    ...
    results = pool.map(
        partial(
            find_motifs_in_chrom, files=[tf_motif_filename, atac_peaks_filename, radius]
        ),
        CHROMOSOMES,
    )
    pool.close()
    ...
    if len(results_matching_motif) > 0:
        ...
        if overall_g_H >= 0:
            return [
                float(overall_g_h + 1) / (overall_g_H + 1),
                (overall_g_h + 1),
                (overall_g_H + 1),
...
def main():
    parser = argparse.ArgumentParser(
        description="This script analyzes ATAC-Seq and GRO-Seq data and produces various plots for further data analysis.",
        epilog="IMPORTANT: Please ensure that ALL bed files used with this script are sorted by the same criteria.",
    )
    parser.add_argument(
        "-e",
        "--atac-peaks",
        dest="atac_peaks_filename",
    ...
    if input_file_statinfo.st_size != 0:
        ...
        raise ValueError(
            "The specified annotation file (e.g. ATAC peaks file) is empty. Exiting."
        )

    if not os.listdir(args.tf_motif_path):
        raise NameError(
            "The specified motif directory is empty or does not exist. Please check that there are motif files in the directory specified by the -m/--motif-path argument."
        )

    if os.path.exists(args.output_dir) is False:
        raise NameError(
            "The specified output directory does not exist. Please specifiy a valid directory."
        )
    ...
    if args.genome:
        ...
        raise ValueError(
            "Either -g/--genome or -c/--chromosomes argument must be specified."
        )
    ...
    print(
        "ATAC mean width: %d bp (std: %d bp). Determined an evaluation radius of %d bp"
    ...
    for filename in motif_filenames:
        ...
        if os.path.getsize(filename) > 0 and os.path.basename(filename).endswith(
            tuple([".bed", ".BedGraph", "bedgraph", ".txt"])
        ):
            [md_score, small_window, large_window, motif_site_count, heat] = (
                get_md_score(
                    filename,
                    int(args.mp_threads),
                    args.atac_peaks_filename,
                    CHROMOSOMES,
                    args.radius,
                )
            )
            print(
                "The MD-score for the provided BED regions vs %s is %.6f"
                % (filename_no_path, md_score)
            )
            motif_stats.append(
                {
                    "motif_file": filename_no_path,
                    "md_score": md_score,
