...
def get_differential_md_scores(diff_params):
    ...
    if is_chip:
        if (n1 <= 70) & (n2 > 70):
            ...
            z_value = p1 - p2
            print(
                "There were not enough regions to calculate the differential MDS for motif %s."
                % label
            )
        else:
            z_value = (p1 - p2) / np.sqrt(
                (control_bootstrap / n1) + (perturbation_bootstrap / n2)
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="This script produces an MA plot of TFs from ATAC-Seq data, for DMSO vs. treatment conditions.",
    ...
    parser.add_argument(
        ...
        dest="p_value",
        metavar="P_VALUE",
        help="p-value cutoff to define which motifs to label in the MA plot. Defaults to 0.00001.",
        default=0.00001,
        required=False,
    )
    parser.add_argument(
        ...
        dest="assay_1",
        metavar="ASSAY_1",
        help='Control file generated from process_atac ending in the extension "md_scores.txt" (e.g. "DMSO", "control", "wildtype").',
        required=True,
    )
    parser.add_argument(
        "-2",
        ...
        metavar="ASSAY_2",
        help=(
            "Perturbation file generated from process_atac ending in the "
            'extension "md_scores.txt" (e.g., "doxycyclin", "p53_knockout").'
        ),
        required=True,
    )
    parser.add_argument(
        ...
        dest="label_1",
        metavar="LABEL_1",
        help="Label for the MA plot title corresponding to assay 1",
        required=False,
    )
    parser.add_argument(
        "-n",
        ...
        dest="label_2",
        metavar="LABEL_2",
        help="Label for the MA plot title corresponding to assay 2",
        required=False,
    )
    parser.add_argument(
        "-w",
        ...
        dest="window_size",
        metavar="WINDOW",
        help="Label for the MA plot title corresponding to window size (str). Default = '3kb'",
        type=str,
        default="3kb",
        required=False,
    )
    parser.add_argument(
        ...
        dest="gen_barcode",
        action="store_true",
        help="Generate a barcode plot for each significant motif",
        default=False,
        required=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        dest="threads",
        metavar="THREADS",
        help="Number of threads for multi-processing. Defaults to 1.",
        default=1,
        required=False,
    )
    parser.add_argument(
        ...
        dest="chip",
        action="store_true",
        help="If the input is ChIP data, it may be useful to specify this flag as it will change the variance calulation because a large difference in sites between control and treatment will be expected.",
        default=False,
        required=False,
    )
    parser.add_argument(
        ...
        dest="global_norm",
        action="store_true",
        help="When specified, output barcodes will be normalized according to total number of motif hits throughout the genome (i.e. total significantly called regions from FIMO scan).",
        default=False,
        required=False,
    )
    ...
    differential_stats_file = open(
        "%s/%s_vs_%s_differential_md_scores.txt"
        % (args.output_dir, assay_1_prefix, assay_2_prefix),
        "w",
    )
    for stat in sorted_differential_stats:
        differential_stats_file.write(
            "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n"
            % (
                stat["motif_name"],
                stat["p_value"],
                stat["total_nr_motifs"],
                stat["control_peaks"],
                stat["perturbation_peaks"],
                stat["control_md_score"],
                stat["perturbation_md_score"],
                stat["delta_md"],
            )
        )
    differential_stats_file.close()

    sorted_differential_stats_sorted = [
        item for item in sorted_differential_stats if item["delta_md"] != 0
    ]
    ...
    np_control, np_perturbation, np_labels = (
        np.array(control),
    ...
    fig, ax = plt.subplots()
    ax.scatter(
        nr_peaks, delta_md, s=sizes, edgecolor="white", linewidth=0.5, color=colors
    )
    texts = []
    ...
    plt.title(
        "MA for %s vs. %s MD-scores\n(p-value cutoff: %.2E)"
        % (label_1_str, label_2_str, P_VALUE_CUTOFF),
        fontsize=12,
    )
    plt.xlabel(
        "$\log_2$(Sum #peaks overlapping " + args.window_size + " window)", fontsize=14
    )
    ...
    if args.gen_barcode:
        ...
        for relevant_tf in most_relevant_tfs:
            ...
            control_bc_data = np.array(control_barcodes[relevant_tf].split(";")).astype(
                float
            )
            perturbation_bc_data = np.array(
                perturbation_barcodes[relevant_tf].split(";")
            ).astype(float)
            ...
            if args.global_norm:
                ...
                control_bc_data = [
                    x / (control_total_motifs[relevant_tf]) for x in control_bc_data
                ]
                perturbation_bc_data = [
                    x / (perturbation_total_motifs[relevant_tf])
                    for x in perturbation_bc_data
                ]
            ...
            ax0.text(
                ...
                HISTOGRAM_BINS / 2,
                "N(total) = %d\nMD-score = %.3f"
                % (control_nr_peaks[relevant_tf], control_mds[relevant_tf]),
                ha="center",
                size=18,
                zorder=0,
            )
            ax0.text(
                HISTOGRAM_BINS / 2, -10, label_1_str, ha="center", size=18, zorder=0
            )
            ...
            ax1.text(
                ...
                HISTOGRAM_BINS / 2,
                "N(total) = %d\nMD-score = %.3f"
                % (perturbation_nr_peaks[relevant_tf], perturbation_mds[relevant_tf]),
                ha="center",
                size=18,
                zorder=0,
            )
            ax1.text(
                HISTOGRAM_BINS / 2, -10, label_2_str, ha="center", size=18, zorder=0
            )

            plt.tight_layout()
            plt.savefig(
                "%s/%s_barcode_%s_vs_%s.png"
                % (args.output_dir, relevant_tf, assay_1_prefix, assay_2_prefix),
                dpi=600,
            )
